...
class AppUtils private constructor() {
    ...
    companion object {
         ...
         * 判断版本号
         */
        fun checkVersion(newVersion: String, versionName: String): Boolean {
            val v1 = newVersion.split(".")
            val v2 = versionName.split(".")
            //防止格式不对直接返回false
            if (v1.size != 3 || v2.size != 3) {
                return true
            }
            if (v1[0].toInt() > v2[0].toInt() || v1[1].toInt() > v2[1].toInt() || v1[2].toInt() > v2[2].toInt()) {
                return false
            }
            return true
        }
        ...
        fun isForeground(context: Context?): Boolean {
            if (context != null) {
                ...
                if (processes.isNullOrEmpty()) return false
